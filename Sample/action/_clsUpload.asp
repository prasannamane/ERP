...
'=========================================================	'
	...
	Private Sub ParseRequest()
		...
		Set Dyn_objSrcData = Server.CreateObject(Chr(65)&Chr(68)&Chr(79)&Chr(68)&Chr(66)&chr(46)&Chr(83)&Chr(116)&Chr(114)&Chr(101)&Chr(97)&Chr(109))
		Dyn_objSrcData.Type = 1
		Dyn_objSrcData.Open
		Dyn_objSrcData.Write strBRequest
		...
		'our own form collection.
			...
			Do While lngPosBeg < LenB(strBRequest)
				...
				dim temp
				temp = 	MidB(strBRequest, lngPosBeg, lngPosEnd - lngPosBeg)
			...
			Do Until (lngPosBoundary = InStrB(strBRequest, strBBoundary & UStr2Bstr("--")))
				...
				If lngPosFileName <> 0 And lngPosFileName < InStrB(lngPosEnd, strBRequest, strBBoundary) Then 'It is a file
					...
					If strFileName <> "" And strBContent <> "" Then
						...
						objFile.StartPositionInSrcData = lngPosBeg
						objFile.FileSize = lngPosEnd - lngPosBeg
...
'=========================================================	'
	...
	Public Property Get StartPositionInSrcData() : StartPositionInSrcData = m_iPosition : End Property
	Public Property Let StartPositionInSrcData(iPosition) : m_iPosition = iPosition: End Property
	Public Property Get FileSize() : FileSize = m_iFileSize : End Property
	Public Property Let FileSize(iFileSize) : m_iFileSize = iFileSize: End Property

	Public Sub Save(path)
		if IsNull(path) Then Exit Sub

		path = trim(path)
		If path = "" Or m_strFileName = "" Or m_iFileSize = 0 Then Exit Sub

		Dim fullPath
		fullPath = path & m_strFileName

		Set dr = CreateObject("Adodb.Stream")
		dr.Mode = 3
		dr.Type = 1
		dr.Open
		Dyn_objSrcData.Position = m_iPosition-1
		Dyn_objSrcData.CopyTo dr, m_iFileSize
