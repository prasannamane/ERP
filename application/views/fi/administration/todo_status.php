             ...
             foreach ($tasktypelist as $tasktypelist_dtls)
              ...
              <?php $i++; } ?>
               <tr class="tr_clone">
                <td><?=$i?></td>
                <td>
                  <input type="text" class="form-control lsttodoname" name="lsttaskname" id="lsttaskname">
                </td>
                <td>
...
$('body').on('change','.taskname',function(){
    ...
    //alert("taskname--"+taskname+" hdntaskid--"+hdntaskid);
        ...
        if(taskname!="")
        {
                  $.ajax({
                  type: 'POST',
                  url: '<?=site_url('fi_home/updtodostatusinfo')?>',
...
$('body').on('change','.taskcolor',function(){
    ...
    //alert("taskcolor--"+taskcolor+" hdntaskid--"+hdntaskid);
            ...
            $.ajax({
            type: 'POST',
            url: '<?=site_url('fi_home/updtodostatuscolorinfo')?>',
...
<script type="text/javascript">
    $(document).ready(function(){
        $('body').on('change','.lsttodoname',function(){
            var taskname    = $(this).val();
            ...
            if(taskname!="")
            {
                $.ajax({
                    type: 'POST',
                    url: '<?=site_url('Administration/inserttodostatusinfo')?>',
       ...
       if(r == true)
         {
                  $.ajax({
                  type: 'POST',
                  url: '<?=site_url('fi_home/deltodostatusinfo')?>',
                  data: {delId:delId},
                  dataType: 'text',
                  beforeSend: function() {
                      // setting a timeout
