...
<div class="box-body" style="">                    
  ...
  <div class="table-responsive" id="topsubtask">
    <table class="table table-hover no-margin customtable">
      <thead>
          <th>#</th>
          <th>Name</th>  
          <th>Due Date</th>                                               
...
$('body').on('change','.taskname',function(){
    ...
    //alert("taskname--"+taskname+" hdntaskid--"+hdntaskid);  

        if(taskname!="")
        {
                $.ajax({
                type: 'POST',
                url: '<?=site_url('fi_home/updtatsksinfo')?>', 
...
$('body').on('change','.taskcolor',function(){
    ...
    var tmptaskcolor= $(this).parents('.tr_clone').find('.taskcolor');  
    //alert("taskcolor--"+taskcolor+" hdntaskid--"+hdntaskid);  

            $.ajax({
            type: 'POST',
            url: '<?=site_url('fi_home/updtatskscolorinfo')?>', 
            ...
            success: function(data) {//alert(data);
               ...
               if(data=="isexists")
                ...
                {
                   //$('#taskallrows').load(location.href + ' #taskallrows');
                   //$('#taskallrows .clrtd').load(location.href + ' #taskallrows .clrtd');
                   $('.fadeMe').hide();
                   loadtaskdetails(hdntaskid);
                  // $('.taskrow').removeClass('active-task'); 
                   //actclr.addClass('active-task');
                   //window.location.href='<?//=site_url('fi_home/administration_task')?>';
...
$('body').on('change','.lsttaskname',function(){
         ...
         var taskclr= $(this).parents('.tr_clone').find('.lsttaskcolor');

          if(taskname!="")
          {
                  $.ajax({
                  type: 'POST',
                  url: '<?=site_url('Administration/inserttsksinfo')?>', 
       ...
       if(r == true)
         {
                  $.ajax({
                  type: 'POST',
                  url: '<?=site_url('fi_home/deltasksinfo')?>', 
                  data: {delId:delId},
                  dataType: 'text',
                  beforeSend: function() {
                      // setting a timeout
...
<script type="text/javascript">
 $(document).ready(function()
 {
    //$('.tskrow tr:first-child').trigger('dblclick');
     $('.tskrow').removeClass('active-task'); 
     $('.tskrow tr:first-child').addClass('active-task');
     var hdntaskid= $('.tskrow .active-task').find('.hdntaskid').val();
     loadtaskdetails(hdntaskid);
 });

 function loadtaskdetails(hdntaskid)
 {
            $.ajax({
            type: 'POST',
            url: '<?=site_url('Administration/loadsubtaskinfo')?>', 
            data: {hdntaskid:hdntaskid},
            dataType: 'html',
            beforeSend: function() {
                // setting a timeout
                $('.fadeMe').show();
               //alert("before hdntaskid--"+hdntaskid);
            },
            success: function(data) {//alert(data);

                if(data!="")
                {
                   //$('#topsubtask').load(location.href + ' #topsubtask');
                   $('#subtskrow').html(data);
                   $('.fadeMe').hide();
                }else if(data=="error"){
...
<script type="text/javascript">
$(document).ready(function(){
$('body').on('dblclick','.taskrow',function(){
    
     $('.taskrow').removeClass('active-task'); 
     $(this).addClass('active-task');
     var hdntaskid= $(this).find('.hdntaskid').val();
     //alert("hdntaskid--"+hdntaskid); 

            $.ajax({
            type: 'POST',
            url: '<?=site_url('fi_home/loadsubtaskinfo')?>', 
            data: {hdntaskid:hdntaskid},
            dataType: 'html',
            beforeSend: function() {
                // setting a timeout
...
<script type="text/javascript">
  function fndelsubtasks(delId)
   {
      var r = confirm("Do you want delete this subtask..?");
       if(r == true)
         {
            var taskId= $('#subtskrow').find('.hdnsbtaskid').val();
                  $.ajax({
                  type: 'POST',
                  url: '<?=site_url('fi_home/fndelsubtasksinfo')?>', 
                  data: {delId:delId},
                  dataType: 'text',
                  beforeSend: function() {
                      // setting a timeout
...
<script type="text/javascript">
$(document).ready(function(){
$('body').on('change','.lstsubtaskname',function(){

         var subtaskname= $(this).val();
         var subduedt= $(this).parents('.tr_clone').find('.lstsubduedt').val();
         var taskId= $('.tskrow .active-task').find('.hdntaskid').val();
          //alert("subtaskname--"+subtaskname+" subduedt--"+subduedt+" taskId--"+taskId);
            $.ajax({
            type: 'POST',
            url: '<?=site_url('fi_home/insertsubtsksinfo')?>', 
            data: {subtaskname:subtaskname,subduedt:subduedt,taskId:taskId},
            dataType: 'text',
            beforeSend: function() {
                // setting a timeout
                $('.fadeMe').show();
...
<script type="text/javascript">
function fnupdatesubtaskinfo(inptxtval,substkid,fieldnm,taskId)
  {
            $.ajax({
            type: 'POST',
            url: '<?=site_url('fi_home/fnupdtesubtasksinfo')?>', 
            data: {inptxtval:inptxtval,substkid:substkid,fieldnm:fieldnm},
            dataType: 'text',
            beforeSend: function() {
                // setting a timeout
                $('.fadeMe').show();
