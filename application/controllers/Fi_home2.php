<?php
    ...
    class Fi_home extends CI_Controller {
        ...
        public function index() {
            ...
            $data['event']   = $this->db->select('cus_id,event_id,event_type,event_name,event_date,event_note')->where($where_con_event)->get('events_register')->result_array();
//print_r($this->db->last_query());    
            $where_con_task='ic.cust_id !="" AND`task_date_started` <= DATE_ADD(CURDATE(), INTERVAL 5 DAY) ORDER by i.task_id DESC LIMIT 3';
            $data['task']  = $this->db->select('ic.invoice_id,ic.cust_id,i.task_user,i.task_id,i.task_date_started,a.name as type_name,ad.name as sub_name,i.task_note')->join("adm_task_type a", "i.task_type=a.id",'left')->join("adm_subtask_type ad", "i.sub_task_type=ad.id",'left')->join("invoices_create ic","i.invoice_id=ic.invoice_id",'left')->where($where_con_task)->get('invoice_task i')->result_array();
            ...
            $data['missing']  = $this->db->select('cus_id,event_id,event_type,event_name,event_date,event_note')->where($where_con_missing)->get('events_register')->result_array();

            $where_con_rem='iteam_check = "1" AND  note_datetime >= CURDATE() AND note_datetime <= DATE_ADD(CURDATE(), INTERVAL 2 WEEK) ORDER by note_datetime ASC LIMIT 3';
            $data['rem']  = $this->db->select('*')->where($where_con_rem)->get('customer_appointment')->result_array();
		...
		public function all_upcoming_event() {
			if(!isset($this->session->fi_session)){
				redirect('/','refresh');
			}
			...
			$where_con='event_date >= CURDATE() AND event_date <= DATE_ADD(CURDATE(), INTERVAL 2 WEEK) ORDER by event_id DESC';
			$data['event']  = $this->db->select('cus_id,event_id,event_type,event_name,event_date')->where($where_con)->get('events_register')->result_array();
...
public function all_upcoming_task($id) {
    ...
    $where_con_task='`task_date_started` <= DATE_ADD(CURDATE(), INTERVAL 5 DAY) ORDER by i.task_id DESC ';
...
public function cust_search()
{
	...
	public function generalinfo()
	{
		...
		// $event_session=$this->session->userdata('event_page');
		// // print_r($event_session);die;
		// if ($event_session==1) {
		// 	$result_event=$this->event_all_data_save();
		// }
		$this->session->unset_userdata('event_page', '');
...
*/
	...
	public function vendor_genral_info()
	{
		...
		$data['contact'] 	  = $this->db->where('cat_id',1)->get('sub_categories')->result_array();
		$data['venders'] = $this->AdminModel->vendor_search_data();
		// echo "<pre>";print_r($data['venders']);die;
		$data['single_cust'] = $this->AdminModel->vendor_search_data()[0];
		$data['ap_cat']  = $this->db->where('cat_id',5)->get('sub_categories')->result_array();
        $data['ap_subcat']  = $this->db->where('cat_id',4)->get('sub_categories')->result_array();
	...
	public function contract_pdf()
	{
		...
		$this->load->view('fi/header');
		$this->load->view('fi/sidebar');
		$this->load->view('fi/contract_pdf');
		$this->load->view('fi/footer');
	}
	...
	public function sales_tax_details()
	{
		...
		$this->load->view('fi/header');
		$this->load->view('fi/sidebar');
		$this->load->view('fi/sales-tax-details');
		$this->load->view('fi/footer');
	}
	...
	public function upcoming_events_report()
	{
		...
		$this->load->view('fi/header');
		$this->load->view('fi/sidebar');
		$this->load->view('fi/upcoming-events-report');
		$this->load->view('fi/footer');
	}
	...
	public function referredby_report()
	{
		...
		$this->load->view('fi/header');
		$this->load->view('fi/sidebar');
		$this->load->view('fi/referredby-report');
		$this->load->view('fi/footer');
	}
	...
	public function selected_crews_report()
	{
		...
		$this->load->view('fi/header');
		$this->load->view('fi/sidebar');
		$this->load->view('fi/selected-crews-report');
		$this->load->view('fi/footer');
	}
	...
	public function profitrepbyevent_report()
	{
		...
		$this->load->view('fi/header');
		$this->load->view('fi/sidebar');
		$this->load->view('fi/profitrepbyevent-report');
		$this->load->view('fi/footer');
	}
	...
	public function expense_detail_report()
	{
		...
		$this->load->view('fi/header');
		$this->load->view('fi/sidebar');
		$this->load->view('fi/expense-detail-report');
		$this->load->view('fi/footer');
	}
	...
	public function expense_report_by_vendor_report()
	{
		...
		$this->load->view('fi/header');
		$this->load->view('fi/sidebar');
		$this->load->view('fi/expense-report-by-vendor-report');
		$this->load->view('fi/footer');
	}
	...
	public function expense_detail_report_by_account_report()
	{
		...
		$this->load->view('fi/header');
		$this->load->view('fi/sidebar');
		$this->load->view('fi/expense-detail-report-by-account-report');
		$this->load->view('fi/footer');
	}
  ...
  public function admin_packages()
	...
	public function administration_user_rights()
	{
		...
		$data['error']    = $this->session->flashdata('error');
		$data['success']  = $this->session->flashdata('success');
		// $data['user'] 	  = $this->db->WHERE('main_user !=',1)->get('users')->result_array();
		
		$data['user'] 	  = $this->db->order_by('main_user', 'DESC')->get('users')->result_array();
		// echo "<pre>";print_r($data);die;
		$this->load->view('fi/header');
		$this->load->view('fi/sidebar');
	...
	public function activate_deactivate_account()
	{
	    ...
	    $updated = $this->AdminModel->update_tbl("users", array("status" => $new_status), array("id" => $id));
	...
	public function administration_user_create()
	{
		...
		$this->load->view('fi/header');
		$this->load->view('fi/sidebar');
		$this->load->view('fi/new_user_create',$data);
		$this->load->view('fi/footer');
	}
		...
		if(isset($_POST['Save']))
		 {
		 	  ...
		 	  $str = $custregsqlrow->cus_acc_no;//+1;
			  $strlength = 7;
			  $str_pad = 0;
...
$this->session->set_flashdata('success', 'Customer updated successfully ..!');
//redirect('fi_home/custevents');
redirect('fi_home/vendor_search');
}
...
redirect('fi_home/vendor_genral_info');
	...
	{
		$cName = $this->input->post('name');
		$this->AdminModel->allGeneralVendInfo($cName);
	}
  ...
  public function addeventinfo()
	{
		if(!isset($this->session->fi_session)){
		   ...
		   //echo "<pre>";print_r($_POST);die;
			 // print_r("2".$this->input->post('hdneventId'));die;
			if(isset($_POST['Save']))
			{

				 //$this->db->where("event_id",$this->input->post('cuss_id'));
				 // print_r($this->input->post('hdneventId'));die;
		...
		// end
		 ...
		 if($upevnamenrow>0)
		 {
		  ...
		  // print_r($this->input->post('hdneventId'));die;
		  // print_r($this->input->post('atype'));die;
		 $this->db->where("event_id",$this->input->post('hdneventId'));
		 if($this->db->delete('crew_availability'))
			 {
				 $crw= array();
  ...
  public function administration_crewavailability()
	{
		...
		$data['success']  = $this->session->flashdata('success');
		// $data['user'] 	  = $this->db->get('user_register')->result_array();
		$data['template_data'] 	  = $this->db->get('adm_crewavailability_info')->result_array();
  ...
  public function edit_crewavailability($id)
	{
		...
		redirect('fi_home/edit_crewtemplate');
	}
 ...
 public function add_newcrewavailabilities()
	{
		...
		//$data['temp_cat'] 	  = $this->db->where('cat_id',37)->get('sub_categories')->result_array();

		$this->load->view('fi/header');
		$this->load->view('fi/sidebar');
		$this->load->view('fi/administration_addcrewavailability',$data);
		$this->load->view('fi/footer');
	}
	      ...
	      $filesCount = count($_FILES['crewavl']['name']);
	        ...
	        for($i = 0; $i < $filesCount; $i++){
	            ...
	            if($this->upload->do_upload('crewavl')){
	             ...
	             $this->email->from('info@gmail.com', 'ERP SYSTEM- Crew Availability');
	             $this->email->to($to);
...
*/
    ...
    if(isset($_FILES['crewavl']))
    {
        if(count($_FILES['crewavl']['name']) > 0)
        {
        	//echo count($_FILES['crewavl']['name']); die;
            for($x=0; $x<count($_FILES['crewavl']['name']); $x++)
            {
                $file = fopen($_FILES['crewavl']['tmp_name'][$x],"rb");
                $data = fread($file,filesize($_FILES['crewavl']['tmp_name'][$x]));
                fclose($file);
                $data = chunk_split(base64_encode($data));
                $name = $_FILES['crewavl']['name'][$x];
...
public function administration_terms()
 ...
 public function administration_task()
 {
		...
		$data['error']    = $this->session->flashdata('error');
		$data['success']  = $this->session->flashdata('success');
		$data['tasktypelist'] 	  = $this->db->order_by('id ASC')->get('adm_task_type')->result_array();
		$this->load->view('fi/header');
		$this->load->view('fi/sidebar');
...
public function viewpostdate()
 ...
 public function admin_taskstatus()
 {
		...
		$data['error']    = $this->session->flashdata('error');
		$data['success']  = $this->session->flashdata('success');
		$data['tasktypelist'] = $this->db->order_by('id ASC')->get('adm_task_status')->result_array();
		$this->load->view('fi/header');
		$this->load->view('fi/sidebar');
 ...
 public function admin_todotatus()
 {
		...
		$data['error']    = $this->session->flashdata('error');
		$data['success']  = $this->session->flashdata('success');
		$data['tasktypelist'] = $this->db->order_by('id ASC')->get('adm_todo_status')->result_array();
		$this->load->view('fi/header');
		$this->load->view('fi/sidebar');
			...
			if($rid > 0) {
				...
				$url = base_url()."change_password?rid=".$userid;
								 $message = "<html>
															 ...
															 <title>Thank You for Registration - ERP App</title>
															 </head>
															 <body>
															 <div  style='width:auto; height:405px; padding-left:25px'>
																<div>
																		<p>Hello ".",</p>
																		<p>Thank you for registering with <strong>ERP App</strong></p>
																		<p>Please click on link For Create Your Password.</p>
																		<p><b><a href=".$url.">Click Here</a></b></p>
																		<br>
																		<br>
																		<p> Thank You!</p>
																</div>
								...
								$this->session->set_flashdata('success',"New User Register Succfully..!!");
				 				redirect('fi_home/administration_user_create');
						 }
						 else
						 {
							 $this->session->set_flashdata('error',"Something Went wrong..!!");
